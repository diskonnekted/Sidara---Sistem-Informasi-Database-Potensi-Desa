...
$potential = $slug !== '' ? sidara_find_potential($slug) : null;

if ($potential === null) {
    http_response_code(404);
    echo 'Potensi tidak ditemukan.';
    exit;
}
...
<head>
  <meta charset="UTF-8">
  <title><?php echo htmlspecialchars($potential['title'], ENT_QUOTES, 'UTF-8'); ?> - SIDARA</title>
  ...
  <div class="min-h-screen flex flex-col">
    <header class="bg-emerald-900 text-emerald-50">
      <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <a href="peta.php" class="hidden sm:inline-flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-full bg-emerald-800 border border-emerald-600">
              ...
              <div class="grid sm:grid-cols-3 gap-4 text-sm">
                <div class="bg-slate-50 rounded-2xl border border-slate-100 p-3.5 space-y-1.5">
                  ...
                  <p class="text-[11px] text-slate-500">
                    <?php echo htmlspecialchars($potential['district_name'], ENT_QUOTES, 'UTF-8'); ?>, Banjarnegara
                  </p>
                </div>
                <div class="bg-slate-50 rounded-2xl border border-slate-100 p-3.5 space-y-1.5">
                ...
                <div class="bg-slate-50 rounded-2xl border border-slate-100 p-3.5 space-y-1.5">
                  ...
                  <?php if (!empty($potential['latitude']) && !empty($potential['longitude'])): ?>
                    <p class="text-sm font-semibold text-slate-900">
                      <?php echo $potential['latitude']; ?>, <?php echo $potential['longitude']; ?>
                    </p>
                    <p class="text-[11px] text-slate-500">
          ...
          <aside class="space-y-4 md:space-y-5">
            <div class="bg-white rounded-3xl border border-amber-100 p-4 md:p-5 space-y-3">
              ...
              <div class="flex flex-col gap-2">
                <a href="#" class="inline-flex items-center justify-center gap-2 text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl px-3 py-2">
                  <span>Hubungi via WhatsApp</span>
                </a>
                <?php if (!empty($potential['latitude']) && !empty($potential['longitude'])): ?>
                  <a
                    href="https://www.google.com/maps?q=<?php echo $potential['latitude']; ?>,<?php echo $potential['longitude']; ?>"
                    target="_blank"
                ...
                <?php else: ?>
                  <a href="peta.php" class="inline-flex items-center justify-center gap-2 text-xs font-semibold border border-emerald-600 text-emerald-700 rounded-xl px-3 py-2 bg-emerald-50 hover:bg-emerald-100">
    ...
    <nav class="fixed bottom-0 inset-x-0 z-20 md:hidden">
      <div class="mx-auto max-w-md px-4 pb-3">
        <div class="bg-white rounded-2xl shadow-xl border border-amber-100 px-4 py-1.5 flex justify-between">
          <a href="index.php" class="flex flex-col items-center justify-center flex-1 py-1.5 text-slate-500">
          ...
          <a href="peta.php" class="flex flex-col items-center justify-center flex-1 py-1.5 text-slate-500">
            <svg class="w-5 h-5 mb-0.5" viewBox="0 0 24 24" fill="none">
              <path d="M8 7C8 5.89543 8.89543 5 10 5H14C15.1046 5 16.1962 5.89543 16 7V9C16 10.1046 15.1046 11 14 11H10C8.89543 11 8 10.1046 8 9V7Z" stroke="currentColor" stroke-width="1.6"></path>
